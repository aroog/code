...
public class RefinementAnalysis extends AbstractCrystalMethodAnalysis {
	...
	private static final boolean LACK_INFO_PROCESSING_ENABLED2 = false; // Pass2. Pick other typings; see if they work.
...
//			}
	...
	public void afterAllCompilationUnits() {
		...
		if(this.isFirstTime){
			...
			for (MethodDeclaration methodDcl : methodDecls) {
				methodDeclarations.add(methodDcl);
			...
			populateOverriding();
			populateEqualityConstraint(initialTM);
			// for the first time, before any refinement, the analysis needs to show a graph
			setInferenceResult(true);
		}
		
		if(currentTM != null){
			TM copyWorking = currentTM.copyTypeMapping(currentTM.getKey());
			
			Set<Variable> allVars = getPotentialEncapsulatedVars();
			
			// OOGRE Auto mode: No manual refinements
			// Set in the config file, the option OOGREAuto
			if(InferOptions.getInstance().isAutoOOGRE()){
				oogreAuto(allVars, copyWorking, facadeRefModel);
			}
			// User wants to do one or more refinements
			// Then he can run auto mode for the rest of the work
			else{
				//the user is done with the manual refinements
				if(facade.isAuto()){
					//Remove the target variables of this completed refinement from the set of all variable for OOGREAuto
					for (Refinement ref : allRefinements) {
						if(ref.getState().equals(RefinementState.Completed) || ref.getState().equals(RefinementState.MoreInfoNeeded)){
							if(ref instanceof SplitUp){
								allVars.removeAll(ref.getAllVars());
							}
							else{
								Collection<Variable> refVars = ref.getAllVars();
								for (Variable variable : refVars) {
									Variable lhsVar = context.getLhsVar(variable);
									allVars.remove(lhsVar);
								}
							}
						}
					}
					...
					if(refinement!=null){
						...
						if(success) {
							if(refinement!= null) {
								...
								context.addCompletedOperation(refinement);
							}
						}
						else {
		...
		SanityCheckRefs sanityChecker = new SanityCheckRefs();
		sanityChecker.doit();
		...
		if (Config.SAVE_ANNOTATIONS && tm != null) {
			lastSavedTM = tm.copyTypeMapping(tm.getKey());
			...
			lastSavedTM = initialTM.copyTypeMapping(tm.getKey());
		}
	...
	private Set<Variable> getPotentialEncapsulatedVars() {
		Set<Variable> encVarsSet = new HashSet<Variable>();
		for (Variable variable : currentTM.keySet()) {
			if(variable instanceof TACVariable){
				TACVariable fieldVar = (TACVariable)variable;
				IVariableBinding varBinding = fieldVar.getVarDecl();
				ITypeBinding typeBinding = varBinding.getType();
				if(!typeBinding.getQualifiedName().contains("java.lang") && !typeBinding.isTypeVariable()){
					encVarsSet.add(variable);
				}
			}
			else if(variable instanceof SourceVariable){
				SourceVariable srcVar = (SourceVariable)variable;
				IVariableBinding binding = srcVar.getBinding();
				if(!binding.isField() && !binding.isParameter()){
					ITypeBinding typeBinding = binding.getType();
					if(!typeBinding.getQualifiedName().contains("java.lang") && !typeBinding.isTypeVariable()){
						encVarsSet.add(srcVar);
					}
				}
			}
		}
	...
	private void oogreAuto(Set<Variable> vars, TM copyWorking, RefinementModel facadeRefModel){
		// Set of variables that cannot be encapsulated (failed for PIO)
		Set<Variable> failedEncapsulatedAUs = new HashSet<Variable>();
		// Set of variables that cannot in PD
		Set<Variable> failedPDAUs = new HashSet<Variable>();
		...
		// Run PushIntoPD heuristics
		for (Variable var : failedEncapsulatedAUs) {
			boolean heuApplied = _afterAllCompilationUnitsHeuristics(var);
			if(heuApplied){
				boolean success = _afterAllCompilationUnitsPostProcessing();
				if (success) {
					...
					// If the operation is unsupported then use add the last valid TM to the list of TMs that should be saved
					setCurrentTM(copyWorking);
					// Add failed target variable to be used for PIR heuristic
					failedPDAUs.add(var);
				}
			}
			else {
			...
			if (lastHeuristic != null ) {
				// TODO: This is probably duplicated!
				// Keep track of the heuristic being generated to display
				// It should get stored only once for each instance of heuristic
				oog.re.Heuristic facadeHeu = RefinementFactory.createFacadeHeuristic(lastHeuristic);
				lastHeuristic.setFacadeOpr(facadeHeu);
				facadeRefModel.addHeuristic(facadeHeu);
			}
		}
		isPIPHeuristic = false;
		// Run PushIntoOnwer heuristics
		for (Variable var : failedPDAUs) {
			boolean heuApplied = _afterAllCompilationUnitsHeuristics(var);
			if(heuApplied){
				boolean success = _afterAllCompilationUnitsPostProcessing();
				if (success) {
				...
				// Post-process to check the validity of a typing
				// If the maximal typing is a valid typing, then get out to save the annotations.
				//						System.out.println("--------------Validating------------");
				//						TM typecheckedTM = typecheckIntermediate(currentTM);
				//						if(typecheckedTM!=null && !typecheckedTM.moreInfoNeeded){
				//							break;
				//						}
			}
			else {
		...
		System.out.println("--------------Validating------------");
		tmppPhases();
		// After heuristics, before any refinement, the analysis needs to show a graph
		setInferenceResult(true);
	}
	...
	private boolean _afterAllCompilationUnitsHeuristics(Variable targetVar) {
		...
		if(targetVar!=null ){
			...
			else if(isPIPHeuristic){
				heuristic = new InferPDHeuristic(targetVar);
			}
			else{
				heuristic = new InferOwnerHeuristic(targetVar);
			}
			...
			TM pickedTM = pickPreferredTM(notDiscardedTMs, null);
	...
	private Refinement _afterAllCompilationUnitsRefinements() {
		...
		while(!refQueue.isEmpty()){
			...
			//Pick the preferred ranked TM
			TM pickedTM = pickPreferredTM(notDiscardedTMs, refinement);
	...
	private boolean _afterAllCompilationUnitsPostProcessing() {
		...
		// Remove owned from AUs with public visibility modifiers
	...
	private boolean tmppPhases(){
		...
		if(LACK_INFO_PROCESSING_ENABLED1 && context.isRefined()) {
			...
			if(!newTM.moreInfoNeeded && typecheckedTM!=null){
				UpdateRefinementStatus();
				setInferenceResult(true);
				savedTMs.add(typecheckedTM);
			}
			else{
				// For fully automated mode or the assisted auto mode
				// Comment out the call to mini phase II to resolve conflicts by the analysis (after Auto mode)
//				if(facade.isAuto() || InferOptions.getInstance().isAutoOOGRE()){
//					System.out.println("--------------Resolvcing conflicts------------");
//					TM resolvedTM = miniPhase2(oldTM2);
//					if(!resolvedTM.moreInfoNeeded){
//						UpdateRefinementStatus();
//						setInferenceResult(true);
//						savedTMs.add(resolvedTM);
//					}
//				}
//				else{
				setInferenceResult(false);
//				}
			}
	...
	private TM miniPhase2(TM oldTM2) {
		...
		boolean done = false;
		for (Variable var : currentConflictVarSet) {
			Set<OType> varSet = newTM.getAnalysisResult(var);
			if(varSet.size()>1){
				...
				while(!worklist.isEmpty()) {
					...
					if(!outTM.isDiscarded) {
						...
						if(!outTM.moreInfoNeeded){
							savedTMs.add(aTyping);
							break;
						}
					}
					else{
						// Reset newTM to be TM that contains conflicts 
						newTM = oldTM2.copyTypeMapping(oldTM2.getKey());
	...
	/**
	 * Method to set the result of the inference analysis
	 * ture it the analysis can find a valid typing
	 * @param result
	 */
	private void setInferenceResult(boolean result) {
		OGraphFacade facade = getFacade();
		facade.setInferenceSuccess(result);
	}

	private void UpdateRefinementStatus() {
		List<BaseOperation> moreInfoNeededRefs = context.getMoreInfoNeededRefs();
		for (BaseOperation operation : moreInfoNeededRefs) {
			operation.setState(RefinementState.Completed);
			setFacadeState(RefinementState.Completed);
		}
	...
	private TM removePublicOwned(TM tm) {
		...
		for (Entry<Variable, Set<OType>> entry : newTM.entrySet()) {
			...
			if(au instanceof SourceVariable){
				...
				if(srcVariable.getBinding().isParameter()){
					if(Modifier.isPublic(srcVariable.getBinding().getDeclaringMethod().getModifiers())){
						...
						while(it.hasNext()) {
							...
							if(oType.getOwner().contains("owned")
								|| (oType.getAlpha()!=null && oType.getAlpha().contains("owned"))
								|| (oType.getInner()!=null && oType.getInner().contains("owned"))){
								it.remove();
							}
						}
			...
			else if(au instanceof TACVariable){
				TACVariable fieldVar = (TACVariable)au;
				IVariableBinding fieldBinding = fieldVar.getVarDecl();
				if(Modifier.isPublic(fieldBinding.getModifiers())){
					Set<OType> auTypeMappings = newTM.getTypeMapping(fieldVar);
					Iterator<OType> it = auTypeMappings.iterator();
					while(it.hasNext()) {
						OType oType = it.next();
						if(oType.getOwner().contains("owned")
...
//							}
		...
		while(isChanged){
			...
			PushIntoOwnedTransferFunctions pushIntoOwnedTFs = new PushIntoOwnedTransferFunctions(this, newTM, null, false);
			for (MethodDeclaration methodDcl : methodDecls) {
	...
	private TM askUser(TM aTM) {
		...
		try {
			...
			if (lastRefinement != null) {
				...
				lastRefinement.setMoreInformation(newTM, mie.getAuSet(),mie.getConflictInstruction(),mie.getSuggestedRefinements());
			}
		}
	...
	private Set<Variable> currentConflictVarSet = new HashSet<Variable>();
	
	private void showUser(TM tm, MoreInformationNeededException min){
		currentConflictVarSet.clear();
		if (lastRefinement != null) {
			lastRefinement.setState(RefinementState.MoreInfoNeeded);
			lastRefinement.setMoreInformation(tm, min.getAuSet(),min.getConflictInstruction(),min.getSuggestedRefinements());
			currentConflictVarSet.addAll(min.getAuSet());
		}
		else{
			lastHeuristic.setState(RefinementState.MoreInfoNeeded);
			lastHeuristic.setMoreInformation(tm, min.getAuSet(),min.getConflictInstruction(),min.getSuggestedRefinements());
			currentConflictVarSet.addAll(min.getAuSet());
		}
	}
	...
	private TM resolveMoreIntoNeeded2(TM aTM) {
		...
		for (Entry<Variable, Set<OType>> entry : newTM.entrySet()) {
			...
			if(suspiciousAU instanceof TACVariable || suspiciousAU instanceof TACNewExpr){
		...
		if(!newTM.isInitial()) {
			...
			if(aNewTM!=null){
				...
				while(isChanged){
					...
					try{
						PushIntoOwnedTransferFunctions pushIntoOwnedTFs = new PushIntoOwnedTransferFunctions(this, aNewTM, null, true);
						for (MethodDeclaration methodDcl : methodDecls) {
							doAnalysis(methodDcl, pushIntoOwnedTFs);
						}
					}
					catch (MoreInformationNeededException min){
						showUser(newTM,min);
						if(lastRefinement!=null){
							context.addMoreInfoNeededRefinement(lastRefinement);
						}
						else{
							context.addMoreInfoNeededRefinement(lastHeuristic);
						}
						newTM.moreInfoNeeded = true;
						if(lastSavedTM==null){
							lastSavedTM = initialTM.copyTypeMapping(initialTM.getKey());
						}
						savedTMs.add(lastSavedTM);
						break;
					}
	...
	private Map<Variable, Set<OType>> generateAPermutation(
			Map<Variable, List<Set<OType>>> conflicVariablesAndSets,
			int[] variableIndexes, Variable[] variables) {
		Map<Variable, Set<OType>> aPerm = new HashMap<Variable, Set<OType>>();
		for (int i=0; i<variables.length; i++) {
			List<Set<OType>> list = conflicVariablesAndSets.get(variables[i]);
			Set<OType> set = list.get(variableIndexes[i]);
			aPerm.put(variables[i], set);
		}
	...
	private Set<TM> applyRefinement(Refinement refinement, TM tm0 ) {
		...
		for(TMSolutionType sol : TMSolutionType.values() ) {
			...
			TM newTM = initializeTM(newTMKey, tm0);
			...
			if(isApplied){
				newTMs.add(newTM);
				counter++;
				if(refinement.getDomainName().equals("PD") && refinement.getDstObject().equals("DUMMY")){
					break;
				}
			}
		}
		...
		for(TM newTM : newTMs) {
			...
			while(isChanged)
			{
				...
				// Run the TFs on the temporary newTM
				PushIntoOwnedTransferFunctions pushIntoOwnedTFs = new PushIntoOwnedTransferFunctions(this, newTM, refinement, false);
	...
	private Set<TM> inferHeuristic(TM tm, Heuristic heuristic) {
		...
		Set<TM> resultTM = new HashSet<TM>();
		List<TM> newTMs = new ArrayList<>();
		...
		while(i>-1) {
			...
			while(isChanged){
				...
				// Run the TFs on the temporary newTM
				PushIntoOwnedTransferFunctions pushIntoOwnedTFs = new PushIntoOwnedTransferFunctions(this, newTM, heuristic, false);
				for (MethodDeclaration methodDcl : methodDecls) {
					doAnalysis(methodDcl, pushIntoOwnedTFs);
					if(newTM.isDiscarded){
						break;
	...
	private void populateOverriding() {
		for (MethodDeclaration methDecl2 : this.methodDecls) {
			...
			if(overriddenMethod != null ) {
				...
				context.addOverridingMethod(overriddenMethod, methodBinding2);
				...
				if(overriddenMethodDeclaration!=null){
					...
					if(parametersList1.size()>0 && parametersList2.size()>0 && parametersList1.size()==parametersList2.size()){
						...
						context.addOverridingMethodParams(overriddenMethod, paramterBindingList1);
					}
				}
				
				if(overriddenMethodDeclaration!=null){
					context.addOverridenMethods(overriddenMethod);
					List<IVariableBinding> overriddenMethodParameters = new ArrayList<IVariableBinding>();
					List<SingleVariableDeclaration> overriddenMethodParametersList2 = overriddenMethodDeclaration.parameters();
					for (SingleVariableDeclaration singleVariableDeclaration : overriddenMethodParametersList2) {
						IVariableBinding paramBinding = singleVariableDeclaration.resolveBinding();
						overriddenMethodParameters.add(paramBinding);
					}
					context.addOverriddenMethodParameters(overriddenMethod.getKey(), overriddenMethodParameters);
				}
			}
		}
...
//			methodDeclaration.accept(new HeuristicOwnedLocalsVisitor());
	...
	private TM initializeTM(TMKey newTMKey, TM tm0) {
		
		// Copy from the base;
		TM tm = tm0.copyTypeMapping(newTMKey);
		
		// XXX. XXX. Do not reuse the same variable; pick a fresh variable name.
		//changeBackCompletedOprs(tm);
		return tm;
	}
	
	// TODO: why not merge these two methods into one
	// TODO: since CompletedRefinements/Heuristcs are OOGRE objects, store directly the
	// owner for the target variable that applyRefinement figured out.
	// this is important for splitup, and pushinto param, etc.
...
//				}
	...
	private TM pickPreferredTM(Set<TM> tmSet, Refinement refinement){
		TM resultTM = null;
		if(refinement == null){
			resultTM = normalSolIndexPicking(tmSet, resultTM);
		}
		else{
			boolean isMain = false;
			Set<Variable> vars = refinement.getVars();
			for (Variable variable : vars) {
				if(variable instanceof TACNewExpr){
					...
					if(enclosingTypeBinding.getQualifiedName().equals(Config.MAINCLASS)){
						isMain = true;
					}
					else{
						isMain = false;
			...
			if(refinement instanceof PushIntoOwned){
				if(isMain){
					resultTM = pioMainSolIndexPicking(tmSet, resultTM);
				}
				else{
					resultTM = normalSolIndexPicking(tmSet, resultTM);
				}
				...
				if(isMain){
					resultTM = pipMainSolIndexPicking(tmSet, resultTM);
				}
				else{
					resultTM = normalSolIndexPicking(tmSet, resultTM);
				}
		...
		if(resultTM!=null)
			setSolIndexForTargetVars(resultTM.getKey().getSolIndex(), refinement);
		return resultTM;
	}

	/**
	 * A method to set the correct solution index of the picked TM for the target variables that are going to be respected later
	 * @param solIndex
	 * @param refinement
	 */
	private void setSolIndexForTargetVars(TMSolutionType solIndex,
			Refinement refinement) {
		if(refinement!=null){
			String solutionIndex = "";
			switch (solIndex) {
			case OWNED:
				solutionIndex = "this.owned";
				break;
			case PD:
				solutionIndex = "this.PD";
				break;
			case OWNER:
				solutionIndex = "owner";
				break;
	...
	private TM pipMainSolIndexPicking(Set<TM> tmSet, TM resultTM) {
		for (TM tm : tmSet) {
	...
	private TM pioMainSolIndexPicking(Set<TM> tmSet, TM resultTM) {
	...
	private TM normalSolIndexPicking(Set<TM> tmSet, TM resultTM) {
		for (TM tm : tmSet) {
