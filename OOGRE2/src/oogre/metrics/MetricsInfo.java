...
public class MetricsInfo implements Comparable<MetricsInfo> {
	...
	private static final int FIELD_O_MULTIPLIER = DEFAULT_MULTIPLIER; 
	private static final int FIELD_A_MULTIPLIER = DEFAULT_MULTIPLIER;
	
	private static final int PARAM_O_MULTIPLIER = DEFAULT_MULTIPLIER;
	private static final int PARAM_A_MULTIPLIER = DEFAULT_MULTIPLIER;

	private static final int RET_O_MULTIPLIER = DEFAULT_MULTIPLIER;
	private static final int RET_A_MULTIPLIER = DEFAULT_MULTIPLIER;

	private static final int OWNED_MULTIPLIER = DEFAULT_MULTIPLIER;
	private static final int PD_MULTIPLIER = DEFAULT_MULTIPLIER;
	private static final int SHARED_MULTIPLIER = DEFAULT_MULTIPLIER;
		...
		for (Entry<Variable, Set<OType>> entry : tm.entrySet()) {
			...
			if(au instanceof SourceVariable){
				...
				if(srcVariable.getBinding().isParameter()){
					...
					if(!srcVariable.resolveType().getQualifiedName().equals(Config.MAINCLASS)){
						variableKind = "v";
					}
					// Variable of type Main is special and it is in <shared>
					else{
						value = new OType("shared", null);
				...
				TACMethod returnVar = (TACMethod)au;
	...
	private void getMetrics(int[] entry, OType oType, int weightO, int weightA, int weightI) {
		...
		// To get numbers comparable to Huang et al., just count owner.
		// We no longer use the metrics for picking between TMs or ranking.
...
//			trackPart(entry, oType.getInner(), weightI);
	...
	private void trackPart(int[] entry, String part, int weight) {
		if (part.equals("shared")) {
			entry[INDEX_SHARED] += 1 * weight;
		}
		else if (part.equals("lent")) {
			entry[INDEX_LENT]+= 1 * weight;
		}
		else if (part.equals("unique")) {
			entry[INDEX_UNIQUE]+= 1 * weight;
		}
		else if (part.equals("p")) {
			entry[INDEX_P]+= 1 * weight;
		}
		else if (part.equals("owner")) {
			entry[INDEX_OWNER]+= 1 * weight;
		}
		else if (part.equals("this.owned")) {
			entry[INDEX_OWNED] += 1 * weight;
		}
		// XXX. How are we going to distinguish between "PD" and "n.PD"?
		else if (part.equals("this.PD")) {
			entry[INDEX_PD] += 1 * weight;
		}
		else  {
	...
	public void displayStats(String nameTM, StringBuilder writer) {
		...
		System.err.println("Total lent = " + this.totalLent );
		System.err.println("Total unique = " + this.totalUnique );
		...
		writer.append(CSVConst.COMMA);
		writer.append(this.totalLent);
		writer.append(CSVConst.COMMA);
		writer.append(this.totalUnique);
	}
