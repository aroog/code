...
public class StringMatcher {
	 ...
	 * "\*" means literal "*", etc. Escaping any other character (including the escape character itself), just results in that character in the pattern. e.g., "
	 * \a" means "a" and "\\" means "\" If invoking the StringMatcher with string literals in Java, don't forget escape
	 * characters are represented by "\\".
	 * 
	 * @param pattern the pattern to match text against
	 * @param ignoreCase if true, case is ignored
	 * @param ignoreWildCards if true, wild cards and their escape sequences are ignored (everything is taken
	 *            literally).
	 */
	...
	public boolean match(String text, int start, int end) {
		...
		while (i < segCount) {
			current = fSegments[i];
			int currentMatch;
			int k = current.indexOf(StringMatcher.fSingleWildCard);
			if (k < 0) {
	...
	private void parseWildCards() {
		...
		while (pos < fLength) {
			...
			case '?':
				...
				buf.append(StringMatcher.fSingleWildCard);
				break;
			default:
				buf.append(c);
			}
	...
	protected boolean regExpRegionMatches(String text, int tStart, String p, int pStart, int plen) {
		while (plen-- > 0) {
			...
			if (!fIgnoreWildCards) {
				...
				if (pchar == StringMatcher.fSingleWildCard) {
					continue;
				}
			}
			if (pchar == tchar) {
